...
@interface YBCamera () <UIGestureRecognizerDelegate>

/**捕获设备，通常是前置摄像头，后置摄像头，麦克风（音频输入）*/
@property(nonatomic)AVCaptureDevice *device;
/**AVCaptureDeviceInput代表输入设备，使用AVCaptureDevice来初始化*/
...
/**有效区域框*/
@property (nonatomic, strong) CAShapeLayer * effectiveRectLayer;
/***/
...
- (void)setEffectiveRect:(CGRect)effectiveRect {
    ...
    if (_effectiveRect.size.width > 0) {
        ...
        if (_effectiveImageView) {
            _effectiveImageView.frame = effectiveRect;
        }
    }
}

- (void)setEffectiveRectBorderColor:(UIColor *)effectiveRectBorderColor {
    _effectiveRectBorderColor = effectiveRectBorderColor;
    _effectiveRectLayer.strokeColor = effectiveRectBorderColor.CGColor;
}
...
- (UIImageView *)effectiveImageView {
    if (!_effectiveImageView) {
        _effectiveImageView = [[UIImageView alloc] initWithFrame:self.effectiveRect];
        _effectiveImageView.contentMode = UIViewContentModeScaleAspectFit;
        [self addSubview:_effectiveImageView];
    }
...
- (YBCaptureFlashMode)getCaptureFlashMode {
    return (YBCaptureFlashMode)self.device.torchMode;
}
